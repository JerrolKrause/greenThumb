    ...
    greenThumb.app.factory("gtGetData", function ($http, $rootScope) {
        ...
        var model = {
            ...
            updateDates: function (obj) {
                ...
                if (data.produce[obj.id].hasOwnProperty('parent')) {
                    //Load the data from the parent item
                    produce = data.produce[data.produce[obj.id].parent];
                    //Now overwrite the parent data with any data present in the child item
                    angular.forEach(data.produce[obj.id], function (value, key) {
                        produce[key] = value;
                    });
                    //No parent item, just load default data    
                } else {
                    produce = data.produce[obj.id];
                }
            ...
            addTasks: function (key1, key2) {
                ...
                //$.each(data.garden.areas[key1].produce[key2].dates, function (key3, value3) {
                angular.forEach(data.garden.areas[key1].produce[key2].dates, function (value3, key3) {
                    ...
                    if (value3.isSame(data.today, 'day') === true) {
                        ...
                        var task = model.formatTasks(key3, data.garden.areas[key1].produce[key2]);
